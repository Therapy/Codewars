...
function sumDigPow(a, b) {
  const firstIndex = a;
  const lastIndex = b - a + 1;

  return Array.apply(null, Array(lastIndex)).map((_, n) => firstIndex + n).filter(value => {
    let str = ('' + value).split('');

    return value == str.reduce((acc, val, i) => acc + Math.pow(val, i + 1), 0);
  });
}
...
function sumDigPow(a, b) {
  var result = [];
  while (a <= b) {
    if (('' + a).split('').reduce((acc, val, i) => acc + Math.pow(+val, i + 1), 0) == a) {
      result.push(a);
    }
    a++;
  }
